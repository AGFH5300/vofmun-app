...
create index if not exists idx_chair_speech_chair
  on public."Chair-Speech" using btree ("chairID") TABLESPACE pg_default;

create index if not exists idx_delegate_speech_delegate
  on public."Delegate-Speech" using btree ("delegateID") TABLESPACE pg_default;

create index if not exists idx_resos_delegate
  on public."Resos" using btree ("delegateID") TABLESPACE pg_default;

create index if not exists idx_chat_rooms_created_by
  on public.chat_rooms using btree (created_by) TABLESPACE pg_default;

create index if not exists idx_chat_rooms_private
  on public.chat_rooms using btree (is_private) TABLESPACE pg_default;

create index if not exists idx_friend_requests_receiver
  on public.friend_requests using btree (receiver_id) TABLESPACE pg_default;

create index if not exists idx_friend_requests_sender
  on public.friend_requests using btree (sender_id) TABLESPACE pg_default;

create index if not exists idx_friend_requests_status
  on public.friend_requests using btree (status) TABLESPACE pg_default;

create index if not exists idx_friendships_user1
  on public.friendships using btree (user1_id) TABLESPACE pg_default;

create index if not exists idx_friendships_user2
  on public.friendships using btree (user2_id) TABLESPACE pg_default;

create index if not exists idx_messages_created_at
  on public.messages using btree (created_at desc) TABLESPACE pg_default;

create index if not exists idx_messages_reply_to
  on public.messages using btree (reply_to) TABLESPACE pg_default;

create index if not exists idx_messages_room_id
  on public.messages using btree (room_id) TABLESPACE pg_default;

create index if not exists idx_messages_user_id
  on public.messages using btree (user_id) TABLESPACE pg_default;

create index if not exists idx_room_members_room_id
  on public.room_members using btree (room_id) TABLESPACE pg_default;

create index if not exists idx_room_members_user_id
  on public.room_members using btree (user_id) TABLESPACE pg_default;
