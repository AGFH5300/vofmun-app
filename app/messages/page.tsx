...
const ChatShell: React.FC = () => {
  ...
  const orderedRoomsForDefaultSelection = useMemo(() => {
    const byLatestMessage = (a: RoomWithDetails, b: RoomWithDetails) => {
  ...
  useEffect(() => {
    ...
    if (!firstRoom) return;
    pendingRoomScrollRef.current = String(firstRoom.id);
    void selectRoom(firstRoom);
  }, [activeRoom, initialChatReady, orderedRoomsForDefaultSelection, selectRoom]);
            ...
            >
              ...
              >
                {activeRoom ? (
                  ...
                  <div className="flex h-full items-center justify-center bg-[#f9f9f9] px-6 text-center text-almost-black-green">
                    <div className="w-full max-w-sm rounded-2xl border border-[#dcc0bd]/30 bg-[#f4f3f3] px-6 py-5">
                      <p className="text-sm font-semibold text-deep-red">No conversations available</p>
                      <p className="mt-1 text-xs text-almost-black-green/65">Start a new chat to begin messaging.</p>
                    </div>
                  </div>
                )}
          ...
          <aside className="hidden min-h-0 w-[270px] flex-col gap-6 overflow-y-auto border-l border-[#dcc0bd]/25 bg-[#f9f9f9] px-5 py-5 lg:flex">
            <div>
              ...
              <div className="mt-3 rounded-xl bg-[#f4f3f3] p-3.5 text-xs text-almost-black-green/75">
                <p className="leading-relaxed">{activeRoom?.description || "Official discussion channel for this room."}</p>
                <p className="mt-3 text-[10px] text-slate-400">{activeRoomMembers.length} Delegates Online</p>
              </div>
            </div>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">Shared Resources</p>
            ...
            <div className="rounded-xl bg-[#6E1D1B]/5 p-3.5">
              ...
              <p className="mt-1.5 text-[10px] italic text-almost-black-green/65">
